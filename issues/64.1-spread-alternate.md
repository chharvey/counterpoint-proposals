...
[1, ["b" |-> 2, "c" |-> 3].*]; %> TypeError: map spread into tuple
[["b" |-> 2, "c" |-> 3].*, 1]; %> TypeError: map spread into tuple
[a= 1, [2, 3].*];                 %> TypeError: tuple   spread into record
[[2, 3].*, a= 1];                 %> TypeError: tuple   spread into record
[a= 1, ["b" |-> 2, "c" |-> 3].*]; %> TypeError: map spread into record
[["b" |-> 2, "c" |-> 3].*, a= 1]; %> TypeError: map spread into record
["a" |-> 1, [2, 3].*];       %> TypeError: tuple  spread into map
[[2, 3].*, "a" |-> 1];       %> TypeError: tuple  spread into map
["a" |-> 1, [b= 2, c= 3].*]; %> TypeError: record spread into map
[[b= 2, c= 3].*, "a" |-> 1]; %> TypeError: record spread into map

[[b= 2, c= 3].*, [1].*];           %> TypeError: tuple spread into record
[["b" |-> 2, "c" |-> 3].*, [1].*]; %> TypeError: tuple spread into map
[[2, 3].*, [a= 1].*];                 %> TypeError: record spread into tuple
[["b" |-> 2, "c" |-> 3].*, [a= 1].*]; %> TypeError: record spread into map
[[2, 3].*, ["a" |-> 1].*];       %> TypeError: map spread spread into tuple
[[b= 2, c= 3].*, ["a" |-> 1].*]; %> TypeError: map spread spread into record
```
