...
val y: Maybe[int] = x.then(\(value) => value + 1);
assert y == Some[int](43);

set x = None[int]();
val z: Maybe[int] = x.then(\(value) => value + 1);
assert z == None[int]();
...
```cpl
val x: Maybe[int] = 42;   %> TypeErrorNotAssignable
val y: Maybe[int] = null; %> TypeErrorNotAssignable

val x: Maybe[int] = Some[int](42); % ok
val y: Maybe[int] = None[int]();   % ok
...
val mut maybe: Maybe[int] = Some[int](42);
...
```cpl
val rec_maybe: (item: int)? = Some[(item: int)]((item= 42));
...
```cpl
val mut x: Result[int] = Ok[int](42);
val y: Result[int] = x.then(\(value) => value + 1);
assert y == Ok[int](43);

set x = Fail[int]();
val z: Result[int] = x.then(\(value) => value + 1);
assert z == Fail[int]();
...
```cpl
val x: Result[int] = 42;                 %> TypeErrorNotAssignable
val y: Result[int] = Exception("oops!"); %> TypeErrorNotAssignable

val x: Result[int] = Ok[int](42);        % ok
val y: Result[int] = Fail[int]("oops!"); % ok
...
```cpl
val mut union: int | Exception = 42;
set union += 1; %> TypeErrorInvalidOperation
...
val mut result: Result[int] = Ok[int](42);
...
```cpl
val rec_result: (item: int)! = Ok[(item: int)]([item= 42]);
...
val rec: (item: int) = rec_result~!; % no TypeError, but unsafe!
...
% None objects
None[anything]() && rhs; %== None()
None[anything]() || rhs; %== rhs
% Fail objects
Fail[anything]("demo") && rhs; %== Fail("demo")
Fail[anything]("demo") || rhs; %== rhs
...
```cpl
val rec_result: (item: int)!(ExceptionEmptyCollection) = Fail[(item: int), ExceptionEmptyCollection](ExceptionEmptyCollection());
%               ^ shorthand for `Result[(item: int), ExceptionEmptyCollection]`
```
