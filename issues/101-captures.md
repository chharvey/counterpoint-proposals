...
func my_fn[my_var, my_vars_list](): void {
%         ^ capture clause
	my_var;       %== "hello"
	my_vars_list; %== ["world"]
	return;
...
func from_below[my_var_below](): str => my_var; % ReferenceError: `my_var_below` is used before it is declared.
...
func my_fn[my_var, my_vars_list](): [str] { % copies and binds new references
	my_var;       %== "hello"
	my_vars_list; %== ["world"]
...
func make_question[my_var](s?: str = my_var): str => """{{ s }}?""";
...
%                            ^ named functions cannot be captured

make_question.();     % prints `"hello"`, returns `"world?"`
make_question.("hi"); % prints nothing,   returns `"hi?"`
make_question.();     % prints `"hello"`, returns `"world?"`
```
In the example above, capturing `my_fn` was not needed, and not even possible, since it is a named function, not a variable. In Counterpoint, named functions (function declarations) are static, so they don’t have pointers. However, as discussed in #46, they cannot be passed around like anonymous functions (function expressions); they must be called when referenced.
...
```cpl
val my_var: str = "hello";
val my_lambda: \() => str = \[ref my_var](): str => my_var; %> ParseError
val my_lambda: \() => str = \[my_var]():     str => my_var; % ok
```
...
% workaround:
val wrapped_counter: mut Dict.<int> = [value= 0]; % or use a more structured type, like `mut interface { value: int; }`
