...
With positional destructuring, the number of entries in the assigned object must match at least the number of assignee variables. Here we’re attempting to assign 3 variables to a destructured tuple of only 2 entries. This restriction also applies to record keys.
```cpl
...
c; %== 3
```
The default value of a destructured variable follows the `=` symbol, just like the default value of a function parameter. Notice here we’ve moved the types to inside the destructure pattern. In positional destructuring, all optional entries must come after all required entries.

For named destructuring, default values don’t need to come last — a required entry may follow an optional one.
```cpl
...
charlie; %== 2
```
(For clarity, and per standard coding style, the whitespace between the destructure key and `=` is omitted.)
...
```cpl
func f(arg= (a: int, b: int, c?: int = 3)): anything {
	arg; %> ReferenceError
	return (a, b, c);
}
...
func g((a= alfa: int, b= bravo?: int = 3, c= charlie: int)): anything {
	a;   %> ReferenceError
	b;   %> ReferenceError
	c;   %> ReferenceError
	return (alfa, bravo, charlie);
}
% typeof g: \((a: int, b?: int, c: int)) => anything
g((a= 1, c= 2)); %== (1, 3, 2)
...
h(arg= (a= 1, c= 2)); %== (1, 3, 2)
```
If the function takes a *named parameter*, we must **alias** the destructured parameter with the `=` symbol (#46). In the examples above, callers of `f` and `h` must assign the provided parameter to the named argument `arg`, even though that identifier isn’t available in the body. Function `g` has no named parameter so this is not applicable.
...
## Syntax Note
- Positional destructuring, declaring variables `a`, `b`, `c` with respective default values `x`, `y`, `z`:
	```cpl
	...
	```
- Named destructuring, declaring variables `x`, `y`, `z` with no default values:
	```cpl
